...
Сохранив изменения в отредактированном файле shablon.tcl и закрыв его, запустим симулятор командой `ns shablon.tcl`. Увидим пустую область моделирования, поскольку ещё не определены никакие объекты и действия (рис. [-@fig:002]).
...
У меня не запустился ns по непонятной причине, была просто бесконечная закгрузка, будто проблема в цикле, но её там не было.
...
![Визуализация простой модели сети с помощью nam](image/3.png){#fig:006 width=70%}
...
![Описание усложненной топологии сети](image/4.png){#fig:009 width=70%}
![Описание усложненной топологии сети](image/5.png){#fig:009 width=70%}
![Описание усложненной топологии сети](image/6.png){#fig:009 width=70%}
...
![Описание кольцевой топологии сети и динамической маршрутизацией пакетов](image/7.png){#fig:7 width=70%}

![Результат](image/8.png){#fig:8 width=70%}
...
Сначала я изменил количество узлов на 5, а 6 узел присоединил отдельно
Вместо UDP создал TCP агент. На стороне принятия использую TCPSink типа DelAck. Поверх TCP работает FTP с 0,5 до 4,5 секунд. также задам разрыв соединения c 1 по 2 модельную секунду между узлами n(0) и n(1)

![Исходный код](image/10.png){#fig:10 width=70%}
![Результат после запуска](image/9.png){#fig:9 width=70%}
